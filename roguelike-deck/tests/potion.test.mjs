...
test("攻撃ポーション使用後、攻撃カードのダメージがボーナス2分増加する", () => {
  ...
  const state = withPotion(
    {
      ...
      enemies: [{ ...base.enemies[0], currentHp: 100, block: 0 }],
    },
    potion,
  );
  ...
  assert.equal(afterCard.enemies[0].currentHp, 93); // 100 - 7 = 93
});
...
test("毒瓶使用で敵にpoison 6が付与される", () => {
  ...
  const state = withPotion(base, potion);
  const result = battle.usePotion(state, 0, rng);
  assert.equal(result.enemies[0].statuses.get("poison"), 6);
  assert.equal(result.run.potions.length, 0);
});
...
test("火炎ポーション使用で敵に8ダメージ", () => {
  ...
  const base = createBattleState();
  const state = withPotion(
    { ...base, enemies: [{ ...base.enemies[0], currentHp: 50, block: 0 }] },
    potion,
  );
  const result = battle.usePotion(state, 0, rng);
  assert.equal(result.enemies[0].currentHp, 42);
  assert.equal(result.run.potions.length, 0);
});
